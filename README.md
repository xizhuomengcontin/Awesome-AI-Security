...
- [**OrcaPromptVault**](https://github.com/Continuum-AI-Corp/OrcaPromptVault) ![GitHub stars](https://img.shields.io/github/stars/Continuum-AI-Corp/OrcaPromptVault?style=social) - 119 prompts, tool schemas and skill definitions from 43 shipping products (ChatGPT, Claude, Claude Code, Codex, Cursor, opencode, Devin, Grok, Gemini and more); 44 of the files are recorded off the wire by a local proxy and carry the command that reproduces them, and every file is marked **captured** or **reported** so a recorded prompt is not mixed with a model-reported one. Inherits 75 files from CL4R1T4S, indexed path by path.
